...
async function getWorkflowRunState(runId) {
  try {
    ...
    core2.info(`getWorkflowRunState#request: ${JSON.stringify(config)}`);
  ...
  Conclusion: ${response.data.conclusion}`
    ...
    if (error3 instanceof Error) {
      core2.error(`getWorkflowRunState: An unexpected error has occurred: ${error3.message}`);
      core2.error(`getWorkflowRunState: An unexpected error has occurred: ${JSON.stringify(config)}`);
...
async function getWorkflowRunFailedJobs(runId) {
  try {
    ...
    if (fetchedFailedJobs.length <= 0) {
      core2.warning(`Failed to find failed Jobs for Workflow Run ${runId}`);
      return [];
    }
  ...
  Jobs: [${jobs.map((job) => job.name)}]`
    ...
    if (error3 instanceof Error) {
      core2.error(
        `getWorkflowRunJobFailures: An unexpected error has occurred: ${error3.message}`
      );
      error3.stack && core2.debug(error3.stack);
    }
    throw error3;
...
async function run() {
  try {
    ...
    init(config2);
    const timeoutMs = config2.runTimeoutSeconds * 1e3;
    let attemptNo = 0;
...
`
  ...
  Conclusion: ${conclusion}`
          ...
          case "timed_out" /* TimedOut */:
            ...
            await logFailureDetails(config2.runId);
      ...
      core3.debug(`Run has not concluded, attempt ${attemptNo}...`);
      await new Promise(
    ...
    if (error3 instanceof Error) {
      ...
      if (!error3.message.includes("Timeout")) {
        core3.warning("Does the token have the correct permissions?");
      }
